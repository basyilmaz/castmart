<x-admin::layouts>
    ...
    <div class="box-shadow rounded bg-white p-4 dark:bg-gray-900">
        <form action="{{ route('admin.marketplace.trendyol.accounts.update', $account) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <x-admin::form.control-group class="mb-4">
                ...
                <x-admin::form.control-group.control
                    type="text"
                    name="supplier_id"
                    :value="old('supplier_id', $account->credentials['supplier_id'] ?? '')"
                    rules="required"
                    :label="'Supplier ID'"
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    Trendyol Satıcı Paneli → Entegrasyon → Satıcı ID (Cari ID)
                </p>
            ...
            <x-admin::form.control-group class="mb-4">
                ...
                <x-admin::form.control-group.control
                    type="text"
                    name="api_key"
                    :value="old('api_key', $account->credentials['api_key'] ?? '')"
                    rules="required"
                    :label="'API Key'"
                    placeholder="API Key"
                />
            ...
            <x-admin::form.control-group class="mb-4">
                ...
                <x-admin::form.control-group.control
                    type="text"
                    name="api_secret"
                    :value="old('api_secret', $account->credentials['api_secret'] ?? '')"
                    rules="required"
                    :label="'API Secret'"
