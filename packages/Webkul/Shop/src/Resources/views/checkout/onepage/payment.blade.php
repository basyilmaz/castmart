{!! view_render_event('CastMart.shop.checkout.onepage.payment_methods.before') !!}
...
{!! view_render_event('CastMart.shop.checkout.onepage.payment_methods.after') !!}
...
@pushOnce('scripts')
    ...
    >
        <div class="mb-7 max-md:last:!mb-0">
            ...
            <template v-else>
                {!! view_render_event('CastMart.shop.checkout.onepage.payment_method.accordion.before') !!}
                ...
                <x-shop::accordion class="overflow-hidden !border-b-0 max-md:rounded-lg max-md:!border-none max-md:!bg-gray-100">
                    ...
                    <x-slot:content class="mt-8 !p-0 max-md:mt-0 max-md:rounded-t-none max-md:border max-md:border-t-0 max-md:!p-4">
                        <div class="flex flex-wrap gap-7 max-md:gap-4 max-sm:gap-2.5">
                            ...
                            >
                                {!! view_render_event('CastMart.shop.checkout.payment-method.before') !!}
                                ...
                                >
                                    {!! view_render_event('CastMart.shop.checkout.onepage.payment-method.image.before') !!}
                                    ...
                                    {!! view_render_event('CastMart.shop.checkout.onepage.payment-method.image.after') !!}

                                    <div>
                                        {!! view_render_event('CastMart.shop.checkout.onepage.payment-method.title.before') !!}

                                        <p class="mt-1.5 text-sm font-semibold max-md:mt-1 max-sm:mt-0">
                                            @{{ payment.method_title }}
                                        </p>
                                        
                                        {!! view_render_event('CastMart.shop.checkout.onepage.payment-method.title.after') !!}

                                        {!! view_render_event('CastMart.shop.checkout.onepage.payment-method.description.before') !!}

                                        <p class="mt-2.5 text-xs font-medium text-zinc-500 max-md:mt-1 max-sm:mt-0">
                                            @{{ payment.description }}
                                        </p> 

                                        {!! view_render_event('CastMart.shop.checkout.onepage.payment-method.description.after') !!}
    
                                    </div>
                                </label>

                                {!! view_render_event('CastMart.shop.checkout.payment-method.after') !!}
                ...
                {!! view_render_event('CastMart.shop.checkout.onepage.payment_method.accordion.after') !!}
            </template>
        </div>
    </script>
